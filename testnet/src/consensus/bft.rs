...
pub struct SignedMessage<T> {
 ...
 use std::collections::BTreeMap; // Removed HashSet as it's used with full path in struct field
...
pub struct BftState {
    ...
    // Stores the highest PrepareQC received for each round.
    pub highest_prepare_qc_by_round: BTreeMap<u64, QuorumCertificate>,
    // Stores the highest PrecommitQC received for each round.
    pub highest_precommit_qc_by_round: BTreeMap<u64, QuorumCertificate>,
    // Stores hashes of items that have achieved a CommitQC.
...
impl BftState {
    pub fn new(
        ...
        BftState {
            ...
            known_qcs: BTreeMap::new(), // This might store QCs more generally, or be replaced/refined by specific QC tracking
            local_node_pubkey,
    ...
    // Methods to be implemented/modified
    ...
    pub fn generate_vote(
        ...
        if proposal.round != self.current_round {
            // TODO: Log "generate_vote: Proposal for wrong round"
            return None;
        }
        ...
        if proposal.parent_qc.round >= proposal.round {
            // TODO: Log "generate_vote: Parent QC round not less than proposal round"
            return None;
        }
        if proposal.proposed_triad_header.parent_hash != proposal.parent_qc.certified_item_hash {
            // TODO: Log "generate_vote: Proposal parent hash mismatch with parent QC"
            return None;
        }
        // TODO: More robustly check if proposal.parent_qc is known/valid (e.g., present in self.known_qcs
        //       or matches a specific expected parent QC for the round/proposer). For now, structural check is enough.

        let proposed_header_hash = proposal.proposed_triad_header.hash();

        // 3. Phase-Specific QC Checks for Precommit and Commit votes
        match vote_type {
            VoteType::Prepare => {
                // No prior QC needed for Prepare votes, just valid proposal structure.
            }
            VoteType::Precommit => {
                match self.highest_prepare_qc_by_round.get(&proposal.round) {
                    Some(prepare_qc) => {
                        if prepare_qc.certified_item_hash != proposed_header_hash || prepare_qc.vote_type != VoteType::Prepare {
                            // TODO: Log "generate_vote: Precommit vote requires matching PrepareQC for the item and round."
                            return None;
                        }
                        // Check if the PrepareQC's round matches the proposal's round.
                        // (already implicitly handled if keying highest_prepare_qc_by_round by round)
                    }
                    None => {
                        // TODO: Log "generate_vote: No PrepareQC found for this round to justify Precommit."
                        return None; // No PrepareQC for this round to justify a Precommit.
                    }
                }
            }
            VoteType::Commit => {
                match self.highest_precommit_qc_by_round.get(&proposal.round) {
                    Some(precommit_qc) => {
                        if precommit_qc.certified_item_hash != proposed_header_hash || precommit_qc.vote_type != VoteType::Precommit {
                            // TODO: Log "generate_vote: Commit vote requires matching PrecommitQC for the item and round."
                            return None;
                        }
                    }
                    None => {
                        // TODO: Log "generate_vote: No PrecommitQC found for this round to justify Commit."
                        return None; // No PrecommitQC for this round.
                    }
                }
            }
        }

        // 4. Proposal Content Validation (Placeholder for future checks):
        //    - Is proposed_triad_header internally consistent?
        //    - Are transactions valid?
        //    - Does the proposal extend the state correctly?
    ...
    pub fn process_signed_vote(
        ...
        if current_vote_list.len() >= self.quorum_threshold {
            ...
            // The key for known_qcs might need refinement based on how QCs are used.
            self.known_qcs.insert(votes_map_key, new_qc.clone()); // Keep storing in general known_qcs

            // Update phase-specific highest QCs and handle commitments
            match new_qc.vote_type {
                VoteType::Prepare => {
                    // TODO: Logic for choosing "highest" if multiple PrepareQCs could form in a round (e.g. different items).
                    // For now, assume one proposal per round, so this is the PrepareQC for this round's proposal.
                ...
                VoteType::Commit => {
                    ...
                    // TODO: Potentially trigger advance_round here or after state execution.
                    // self.advance_round(); // Example: advance round upon first commit in current round.
                }
            }
    ...
    /// Advances the BFT state to the next round.
    pub fn advance_round(&mut self) {
        self.current_round += 1;
        // TODO: Implement leader election/rotation logic for the new round.
        //       For now, current_leader_pubkey remains fixed.

        // TODO: Optionally, clear old QCs from highest_prepare_qc_by_round and
        //       highest_precommit_qc_by_round for rounds much older than current_round
        //       to prevent unbounded memory growth if the node runs for many rounds.
        //       Example: self.highest_prepare_qc_by_round.retain(|&r, _| r >= self.current_round - SOME_WINDOW);
        //       For this phase, this cleanup is skipped for simplicity.

        // TODO: Clear relevant vote maps (prepare_votes, precommit_votes, commit_votes) for the new round
        //       or for rounds older than current_round - SOME_WINDOW.
        //       This is important to prevent stale votes from interfering with new round.
        //       For now, if maps are keyed by (item_hash, round), new round entries will be distinct.
        //       However, general cleanup of very old rounds would be good practice.
    }
}
